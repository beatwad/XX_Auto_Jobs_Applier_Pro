...
- Если личная информация пользователя написана на русском языке, оформляй заголовок на русском (например пиши 'Адрес' вместо 'Address' и 'Телефон' вместо 'Phone') и учитывай, 
что пол пользователя — {sex}. В остальных случаях используй английский язык.
...
Для успешного выполнения задачи следуй следующим шагам:  
...
Для каждой записи о работе необходимо указать:
    ...
    4. **Обязанности и достижения**: Опиши ключевые обязанности и значимые достижения, делая акцент на измеримых результатах и конкретных вкладах 
    (напирмер ускорил(а) деплой системы на 25% или повысил(а) рентабельность на 10%)
...
2. **Детали проекта**: Опиши любые значимые достижения или признание, связанные с проектом, например, количество звезд на GitHub или отзывы сообщества.  
